...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>Inventario De Lenguas 🔥</title>
        ...
        <style>
            ...
            body.welcome-page {
                ...
                background-image:
                    radial-gradient(ellipse 130% 100% at 0% -15%, rgba(124, 232, 173, 0.28), transparent 52%),
                    radial-gradient(ellipse 110% 90% at 100% 15%, rgba(249, 223, 248, 0.22), transparent 48%),
                    radial-gradient(ellipse 100% 60% at 50% 105%, rgba(124, 232, 173, 0.12), transparent 55%),
                    var(--welcome-fond, none);
            ...
            .welcome-card {
                ...
                box-shadow:
                    ...
                    0 0 80px color-mix(in srgb, var(--brand-green) 14%, transparent);
            ...
            .welcome-card::before {
                ...
                background: linear-gradient(
                    135deg,
                    color-mix(in srgb, var(--brand-green) 70%, transparent),
                    color-mix(in srgb, var(--brand-rose) 55%, transparent)
                );
            ...
            .welcome-title {
                ...
                line-height: 1.25;
                text-shadow:
                    0 0 40px color-mix(in srgb, var(--brand-green) 45%, transparent),
                    0 0 72px color-mix(in srgb, var(--brand-rose) 35%, transparent),
            ...
            @media (prefers-reduced-motion: no-preference) {
                .welcome-char {
                    animation: welcome-char-wave 1.35s ease-in-out infinite;
                    animation-delay: calc(var(--char-i, 0) * 0.055s);
                }
            }
            ...
            .welcome-btn--primary {
                ...
                box-shadow: 0 6px 28px color-mix(in srgb, var(--brand-green) 42%, transparent);
            }

            .welcome-btn--primary:hover {
                transform: translateY(-1px);
                box-shadow: 0 10px 36px color-mix(in srgb, var(--brand-green) 55%, transparent);
            }

            .welcome-btn--ghost {
                color: var(--brand-rose);
                background: color-mix(in srgb, var(--brand-rose) 8%, transparent);
                border: 2px solid color-mix(in srgb, var(--brand-rose) 65%, transparent);
                box-shadow: 0 0 24px color-mix(in srgb, var(--brand-rose) 12%, transparent);
            }
    ...
    <body
        class="welcome-page welcome-layout min-h-dvh p-6 antialiased lg:p-8"
        style="--welcome-fond: url('{{ asset('images/fond.png') }}'); min-height: 100vh; min-height: 100dvh;"
    >
        @php
            $welcomeTitle = 'Inventario De Lenguas 🔥';
            $welcomeChars = preg_split('//u', $welcomeTitle, -1, PREG_SPLIT_NO_EMPTY) ?: [];
        @endphp

        <div class="welcome-card-wrap shrink-0">
            <div class="welcome-card px-4 py-3 text-center sm:px-5 sm:py-4">
                <h1 class="welcome-title">
                    @foreach ($welcomeChars as $i => $ch)
                        <span class="welcome-char" style="--char-i: {{ $i }}">{{ $ch }}</span>
                    @endforeach
                </h1>
            </div>
        </div>
        ...
        <div class="welcome-card-wrap shrink-0">
            <div class="welcome-card px-4 py-3 sm:px-5 sm:py-4">
                <div class="welcome-actions">
                    @auth
                        <a href="{{ route('menu') }}" class="welcome-btn welcome-btn--panel">
                            Ir al menú
                        </a>
                    @else
